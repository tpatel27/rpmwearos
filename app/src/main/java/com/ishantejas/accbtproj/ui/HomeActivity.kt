package com.ishantejas.accbtproj.ui
...
import com.ishantejas.accbtproj.app.Constants
import com.ishantejas.accbtproj.base.BaseActivity
import com.ishantejas.accbtproj.data.model.DataWorkout
import com.ishantejas.accbtproj.data.model.EventWorkout
import com.ishantejas.accbtproj.data.model.Exercise
import com.ishantejas.accbtproj.data.model.HeartRateData
import com.ishantejas.accbtproj.data.model.Rest
import com.ishantejas.accbtproj.utils.view.clicks
import com.ishantejas.accbtproj.utils.view.gone
import com.ishantejas.accbtproj.utils.view.show
...
import com.ishantejas.accbtproj.R
import com.ishantejas.accbtproj.databinding.ActivityHomeBinding
import com.ishantejas.accbtproj.ui.viewmodels.HeartDataViewModel
...
class HomeActivity : BaseActivity() {
    ...
    private val viewModel: HeartDataViewModel by viewModels()
    private var heartRateCaptured = ""
        ...
        OnMessageReceivedListener {
            String(it.data).apply {
                ...
                if (receivedEvent.avgHeart?.isNotEmpty() == true) {
                    heartRateCaptured = receivedEvent.avgHeart
                    binding.tvHeartRate.text = heartRateCaptured
                    binding.clContainerInforWatch.show()
                }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnSendAws.clicks {
            if (heartRateCaptured.isNotEmpty()) {

                Log.d("some logs", "onCreate: $heartRateCaptured & ${localDateTime()}")
                viewModel.pushHeartRateData(
                    HeartRateData(
                        timestamp = localDateTime(),
                        heartRate = heartRateCaptured.toInt()
                    )
                )
            } else {
                Toast.makeText(this@HomeActivity, "Heart rate value cannot be empty", Toast.LENGTH_SHORT).show()
            }
        }
    }

    @RequiresApi(Build.VERSION_CODES.O)
    private fun localDateTime(): String {
...
//                }
    ...
    private fun pushHeartRateData(heartRateData: HeartRateData) {
//        viewModel.
    }
}